...
const Dropdown = () => {
  ...
  const handleSelection = species => {
    if (selectedSpecies.includes(species)) {
      setSelectedSpecies(selectedSpecies.filter(selectedSpecies => selectedSpecies !== species));
    } else {
  ...
  const handleDone = () => {
    setIsOpen(false);
    // Function call for API here
  };

  return (
    <div className="dropdown-container">
      <button className={`select-species-button ${isOpen ? "open" : ""}`} onClick={() => setIsOpen(!isOpen)}>
        Select a Species...
        <span><AiOutlineDown></AiOutlineDown></span>
      </button>
      {isOpen && (
